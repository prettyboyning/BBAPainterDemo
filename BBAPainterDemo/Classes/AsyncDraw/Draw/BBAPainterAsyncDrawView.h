...
typedef void(^BBAPainterAsyncDrawCallback)(BOOL drawInBackground);
...
// 是否永远使用离屏渲染，默认YES。子类如果不希望离屏渲染必须重写此方法并 重写drawingPolicy为BBAPainterViewDrawingPolicySynchronouslyDraw
...
/**
 * @brief 立即开始重绘流程，无需等到下一个runloop（异步绘制会在下个runloop开始）
 */
- (void)redraw;
...
/**
 * @brief 子类可以重写，并在此方法中进行绘制，请勿直接调用此方法
 *
...
/**
 * @brief 子类可以重写，是绘制即将开始前的回调，请勿直接调用此方法
 *
 * @param asynchronously 当前是否是异步绘制
 */
...
/**
 * @brief 子类可以重写，是绘制完成后的回调，请勿直接调用此方法
 *
 * @param asynchronously 当前是否是异步绘制
...
/**
 * @brief 子类可以重写，用于在主线程生成并传入绘制所需参数
 *
