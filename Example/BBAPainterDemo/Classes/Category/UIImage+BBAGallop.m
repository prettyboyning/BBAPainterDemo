...
#import "BBAPainterCGRectTransform.h"

#define kDegreesToRadian(x) (M_PI * (x) / 180.0)
#define kRadianToDegrees(radian) (radian * 180.0) / (M_PI)

@implementation UIImage (BBAGallop)
...
- (UIImage *)painter_processedImageWithContentMode:(UIViewContentMode)contentMode size:(CGSize)size {
    ...
    CGRect contentModeRect = [BBAPainterCGRectTransform painter_CGRectFitWithContentMode:contentMode
...
- (void)painter_drawInRect:(CGRect)rect contentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = [BBAPainterCGRectTransform painter_CGRectFitWithContentMode:contentMode rect:rect size:self.size];
...
+ (UIImage *)painter_animatedImageWithAnimatedGIFData:(NSData *)data {
    return animatedImageWithAnimatedGIFReleasingImageSource(CGImageSourceCreateWithData((__bridge CFTypeRef) data, NULL));
}

+ (UIImage *)painter_animatedImageWithAnimatedGIFURL:(NSURL *)url {
...
- (UIImage *)painter_flipVertical {
    return [self painter_rotate:UIImageOrientationDownMirrored];
}

- (UIImage *)painter_flipHorizontal {
    return [self painter_rotate:UIImageOrientationUpMirrored];
}
...
- (UIImage *)painter_imageRotatedByDegrees:(CGFloat)degrees {
    return [self painter_imageRotatedByRadians:kDegreesToRadian(degrees)];
}
...
- (UIImage *)painter_rescaleImageToPX:(CGFloat )toPX {
    ...
    return [self painter_rescaleImageToSize:size];
}
