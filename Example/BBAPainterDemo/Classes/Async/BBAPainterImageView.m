...
#import "BBAPainterProxy.h"
#import "BBAPainterUtils.h"
#import "CALayer+BBAPainterTransaction.h"
#import "BBAPainterTransaction.h"

const NSTimeInterval kPainterGIFDelayTimeIntervalMinimumValue = 0.02; // 每帧图片最小的显示时间

@interface BBAPainterImageView ()

/// 显示gif动画图片时，用于播放gif的定时器，定时触发时，完成切换图片，并setNeedDisplay
@property (nonatomic, strong) CADisplayLink* displayLink;

/// 当前是否需要播放动画，当前BBAPainterImageView可见且gif模型不为nil时返回YES
...
- (void)startAnimating {
    if (self.gifImage) {
        ...
        if (!self.displayLink) {
            ...
            BBAPainterProxy *proxy = [BBAPainterProxy proxyWithObject:self];
...
- (NSTimeInterval)frameDelayGreatestCommonDivisor {
    ...
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kPainterGIFDelayTimeIntervalMinimumValue;
    ...
    for (NSNumber* value in delays) {
        scaledGCD = [BBAPainterUtils greatestCommonDivisorWithNumber:lrint([value floatValue] * kGreatestCommonDivisorPrecision) another:scaledGCD];
    }
